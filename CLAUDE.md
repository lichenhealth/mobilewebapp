...
6. **Events feature — NEXT UP** (founder priority). Rich event pages (photos, meaty details — the Posts architecture: `content_type=actionable`, `service_area=events`), tied to Calendar (an Event carries a calendar entry; the calendar block links to the Event page). Founder will supply Figma mocks + notes. Expands event invites to Groups/Communities/Orgs/Places (composer copy already says so; search still member-only).
